        ...
        <>
          {cart.map((item) => (
            <div key={item.product_id} style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 12, background: "white", padding: 12, borderRadius: 8 }}>
              <img src={getImageUrl(item.image)} alt={item.name} style={{ width: 80, height: 60, objectFit: "cover", borderRadius: 6 }} />
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700 }}>{item.name}</div>
                <div>${item.price.toFixed(2)} each</div>
                <div style={{ marginTop: 6 }}>
                  Qty: <input type="number" value={item.qty} min={1} onChange={(e) => updateQty(item.product_id, Math.max(1, parseInt(e.target.value || "1")))} style={{ width: 60 }} />
